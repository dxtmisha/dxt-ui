...
<Meta title='@dxtmisha/en/scripts/7. Commands/dxt-constructor - Component Constructor Scaffolding'/>
...
1. The script identifies the targeted component name passed via the `--name` option, or automatically scans the configured system directory for all component names.
...
To target a specific component (e.g. `Button`), specify its name via the `--name` (`-n`) option:
```bash
npx dxt-constructor --name Button
```

Run `npx dxt-constructor --help` to print the full list of available options.
