...
<Meta title='@dxtmisha/vi/scripts/7. Lệnh/dxt-constructor - Tạo Khung Bộ Dựng Thành Phần'/>
...
1. Tập lệnh nhận dạng tên thành phần mục tiêu được truyền qua tùy chọn `--name` hoặc tự động quét thư mục hệ thống đã được cấu hình để thu thập tất cả tên thành phần.
...
Để nhắm mục tiêu vào một thành phần cụ thể (ví dụ: `Button`), hãy truyền tên thành phần đó qua tùy chọn `--name` (`-n`):
```bash
npx dxt-constructor --name Button
```

Chạy `npx dxt-constructor --help` để in ra danh sách đầy đủ các tùy chọn khả dụng.
