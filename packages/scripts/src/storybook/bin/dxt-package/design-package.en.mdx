...
<Meta title='@dxtmisha/en/scripts/7. Commands/dxt-package - Package Structure Initialization'/>
...
- **Flexible Configuration**: Allows overriding the default package type, templates, and target base directory via command-line options.

## How does it work?

The package initialization workflow streamlines the creation of new packages:

1. The script reads the provided options to determine the package `type` (defaulting to `library`), `templates`, and target `dir`.
...
To apply specific templates, combine them with the type:
```bash
npx dxt-package --type vue --templates "template1,template2"
```

You can also specify a custom base directory:
```bash
