 ...
 * Executes Antigravity CLI commands directly without a system shell.
 *
 * Реализация Google Antigravity AI через CLI (`agy`).
 * Выполняет команды Antigravity CLI напрямую, без системной оболочки.
 *
 * Responsibilities / Ответственности:
 * - Construct Antigravity CLI command / Сформировать команду Antigravity CLI
 * - Execute command without shell / Выполнить команду без оболочки
 * - Return stdout as result / Вернуть stdout как результат
 *
 * Notes / Примечания:
 * - Requires 'agy' CLI tool installed / Требует установленной утилиты 'agy'
 * - API key is passed via environment variable or config / API ключ передается через переменную окружения или конфиг
 */
export class AiAntigravityCliLite extends AiAbstract<{}> {
  ...
  /**
   * Performs content generation request via Antigravity CLI and returns textual result.
   *
   * Выполняет запрос генерации контента через Antigravity CLI и возвращает текстовый результат.
    ...
    return new Promise((resolve) => {
      ...
      try {
        execFile(
          ...
          (error, stdout, stderr) => {
            if (error) {
              console.error('Error executing Antigravity CLI:', stderr || error.message)
              resolve('')
            } else {
              resolve(stdout.trim())
            }
