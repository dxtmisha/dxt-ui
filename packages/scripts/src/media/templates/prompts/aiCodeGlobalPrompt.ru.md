...
9. **Запрет на преждевременный запуск сборки и тестов**: **НЕ запускай** сборку проекта (`npm run build`, `storybook build` и т.д.) или тесты (`npm run test`, `vitest` и т.д.) во время написания кода или промежуточных этапов, пока разработчик явно не подтвердит завершение работы, не попросит протестировать/проверить результат или пока вы явно не перейдете в режим отладки и исправлений (fixes).
