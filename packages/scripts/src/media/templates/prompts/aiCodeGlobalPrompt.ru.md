...
0. **Обязательное глубокое изучение**:
   ...
   - Поверхностное изучение файлов (например, просмотр только названий файлов, сигнатур функций или выполнение быстрого поиска ключевых слов без фактического чтения содержимого файлов через `view_file`) строго запрещено. Ты ОБЯЗАН читать и изучать файлы полностью и досконально, чтобы детально понимать их внутреннюю реализацию и логику.
   ...
   - **БЛОКИРУЮЩИЙ КОНТРОЛЬ ПОСЛЕДОВАТЕЛЬНОСТИ (ПРАВИЛО ХРОНОЛОГИИ):**
     ...
     6. Ты ОБЯЗАН использовать инструмент `view_file` для чтения и изучения ВСЕХ этих файлов (в частности, если для пакетов/проектов, с которыми ты работаешь, указаны файлы типов `ai-types.md` или руководств `ai-developer.md`, а также файлы типов/документации любых зависимостей пакета, указанных в его package.json, ты ОБЯЗАН прочитать их полностью) ДО ТОГО, как вызывать `list_dir` для папок компонентов, писать какие-либо планы/чек-листы или вносить/предлагать изменения в код. Нарушение этой последовательности является критическим нарушением протокола.
...
6. **Архитектурная консистентность**:
   ...
   - Переиспользование инфраструктуры: Всегда проверяй, существует ли необходимый функционал (например, API-запросы, управление состоянием, утилиты) в базовых пакетах/модулях проекта или стандартных библиотеках, прежде чем писать его с нуля. Для этого ты ОБЯЗАН прочитать их соответствующие файлы типов/документации (такие как `ai-types.md` или аналогичные индексные файлы), чтобы изучить весь список экспортируемых хелперов и классов.
