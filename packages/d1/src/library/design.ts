...
export const componentsReg: RegExp = /((d1|dd)-?(accordion|action-?sheet|actions|anchor|arrow|badge|bars|block|button|cell|checkbox|chip|chip-?group|dialog|dummy|field|field-?counter|field-?label|field-?message|grid|grid-?item|horizontal-?scroll|icon|image|input|list|list-?group|list-?item|list-?menu|mask|menu|modal|motion-?axis|motion-?transform|page|progress|ripple|scrollbar|section|select|select-?value|skeleton|tab-?item|tabs|tabs-?navigation|textarea|textarea-?autosize|tooltip|window))(?![\w-])/ig
