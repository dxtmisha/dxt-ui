...
export const componentsReg: RegExp = /((d1|dd)-?(accordion|action-?sheet|actions|anchor|area|arrow|badge|bars|bleed|block|button|button-?group|cell|checkbox|chip|chip-?group|client-?only|container|dialog|divider|draggable-?wrapper|dummy|field|field-?counter|field-?label|field-?message|grid|grid-?item|group|header|horizontal-?scroll|icon|image|input|input-?phone|input-?phone-dial-code|list|list-?group|list-?item|list-?menu|mask|menu|menu-?button|menu-?chip|menu-?country|modal|motion-?axis|motion-?transform|page|page-?area|pagination|progress|radio|ripple|scrollbar|section|select|select-?value|skeleton|snackbar|snackbar-?item|switch|tab-?item|tabs|tabs-?navigation|text-?description|text-?label|textarea|textarea-?autosize|tooltip|window))(?![\w-])/ig
