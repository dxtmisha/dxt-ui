...
import { t as c } from "./D1Button-CLN_jgWU.js";
import { n as l, r as u, t as d } from "./WindowClassesInclude-zm-ITw5d-QVDLqfAL.js";
import { n as f, t as p } from "./D1Scrollbar-D2_rACq8.js";
import { Teleport as m, computed as h, createBlock as g, defineComponent as _, h as v, mergeDefaults as y, onMounted as b, onUnmounted as ee, openBlock as x, ref as S, unref as C, watch as w } from "vue";
import { DesignConstructorAbstract as T, EventItem as E, executePromise as D, frame as O, getElement as k, getMouseClientX as A, getMouseClientY as j, inArray as M, isDomRuntime as N, isEnter as P, isFilled as F, setRef as I } from "@dxtmisha/functional";
//#region ../constructor/dist/window.js
var L = -1, te = class {
	constructor(t) {
		e(this, "x", L), e(this, "y", L), this.element = t;
	}
	is() {
	...
	constructor(t) {
		e(this, "item", S(u.close)), this.element = t;
	}
	isHide() {
	...
	watchCoordinates() {
		return O(() => {
			let e = this.element.getMain();
			if (e) {
				let t = getComputedStyle(e).getPropertyValue(d).includes("--MENU--");
	...
	isMenu(e = this.element.getMain()) {
		if (e) {
			let t = getComputedStyle(e).getPropertyValue(d);
	...
	constructor(r, c, u, d, p, m, g, _, v) {
		...
		}), this.props = r, this.refs = c, this.classDesign = d, this.className = p, this.components = m, this.slots = g, this.emits = _;
		let { ImageConstructor: y = i, ModelConstructor: x = t, ScrollbarConstructor: S = f, TabIndexConstructor: C = o, TextConstructor: T = a, WindowClassesConstructor: E = l, WindowClientConstructor: D = te, WindowCoordinatesConstructor: O = z, WindowElementConstructor: k = re, WindowEmitConstructor: A = U, WindowEscConstructor: j = s, WindowEventConstructor: M = q, WindowFlashConstructor: N = R, WindowHiddenConstructor: P = oe, WindowHookConstructor: F = ne, WindowOpenConstructor: I = G, WindowOriginConstructor: L = V, WindowPersistentConstructor: J = ae, WindowPositionConstructor: Y = B, WindowStaticConstructor: X = H, WindowStatusConstructor: Z = ie, WindowStylesConstructor: Q = W, WindowVerificationConstructor: $ = K } = v == null ? {} : v;
		this.hook = new F(r), this.classes = new E(p), this.element = new k(this.classes, u), this.client = new D(this.element), this.tabIndex = new C(() => this.element.getBody(), void 0, () => !!this.props.autoTabIndex), this.status = new Z(this.element), this.persistent = new J(r, this.classes, this.element), this.flash = new N(r, this.classes), this.coordinates = new O(this.classes, this.element), this.position = new Y(r, this.client, this.element, this.coordinates), this.origin = new L(this.client, this.element, this.position), this.staticMode = new X(r, c, this.element), this.emit = new A(this.classes, this.element, _), this.styles = new Q(this.element, this.coordinates, this.position, this.origin, p), this.open = new I(r, this.client, this.hook, this.element, this.tabIndex, this.status, this.flash, this.coordinates, this.position, this.origin, this.emit, this.styles), this.verification = new $(r, this.persistent, this.classes, this.element, this.staticMode, this.open), this.event = new M(r, this.tabIndex, this.status, this.client, this.flash, this.open, this.verification), this.scrollbar = new S(r, p, m, _), this.image = new y(r, m, _), this.esc = new j(this.open.item, () => this.open.close(), () => {
			var e;
			return !this.props.persistent && ((e = this.props.closeOnEsc) == null ? !0 : e);
		}), this.text = new T(this.props), this.hidden = new P(this.props, this.classes, this.staticMode, this.open), new x("open", this.emits, this.open.item), b(() => {
			w([c.open], () => this.open.set(r.open), { immediate: !0 });
		}), ee(this.stop);
	}
	isMenu() {
	...
	setup(e, { expose: t, emit: n }) {
		let i = n, a = e, o = new Z("d1.window", a, {
			emits: i,
			classes: h(() => ({ main: {
				"d1-window": !0,
				"d1-window--width--custom": F(a.width) && !M(Q.width, a.width),
				[`d1-window--width--${a.width}`]: M(Q.width, a.width),
				"d1-window--height--custom": F(a.height) && !M(Q.height, a.height),
				[`d1-window--height--${a.height}`]: M(Q.height, a.height),
				"d1-window--hide": a.hide,
				[`d1-window--axis--${a.axis}`]: M(Q.axis, a.axis),
				[`d1-window--imagePosition--${a.imagePosition}`]: M(Q.imagePosition, a.imagePosition),
				"d1-window--dense": a.dense,
				[`d1-window--alignment--${a.alignment}`]: M(Q.alignment, a.alignment),
				[`d1-window--origin--${a.origin}`]: M(Q.origin, a.origin),
				[`d1-window--adaptive--${a.adaptive}`]: M(Q.adaptive, a.adaptive),
				...
				[`d1-window--imageSize--${a.imageSize}`]: M(Q.imageSize, a.imageSize)
			} })),
			styles: h(() => {
				var e, t;
				return {
