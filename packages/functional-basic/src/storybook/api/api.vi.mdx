...
<Meta title='@dxtmisha/vi/functional-basic/1. Api'/>
...
Hàm bọc có thể được cấu hình toàn cục cho toàn bộ instance (thông qua `Api.setWrapper` hoặc trong phương thức `Api.setConfig`) hoặc cục bộ cho một yêu cầu duy nhất bên trong đối tượng `ApiFetch`.
