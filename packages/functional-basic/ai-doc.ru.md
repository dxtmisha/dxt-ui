...
- **Структура класса**: Свойства (`public`->`protected`->`private`) -> Конструктор -> Публичные методы -> Защищенные -> Приватные. Внутри каждой группы методов соблюдать порядок: 1) `get` / `set` (геттеры и сеттеры), 2) `is...` / `has...`, 3) `get...` / `set...`, 4) `add...` / `remove...`, 5) `update...` / `reset...`, 6) остальные методы. Внутри каждой подгруппы методы группируются по смысловой взаимосвязи и логическим правилам (например: `min` / `max`, `width` / `height` (`ширина` / `высота`), `x` / `y` / `z`), а непарные методы сортируются по алфавиту.
