...
Formats numbers with measurement units. Supports automatic unit extraction from string.

**Parameters:**
- `value: NumberOrString` — value to format
- `unitOptions?: string | Intl.NumberFormatOptions` — measurement unit or formatting options (optional)
...
- `unitOptions?: 'byte' | 'kilobyte' | 'megabyte' | 'gigabyte' | 'terabyte' | 'petabyte' | Intl.NumberFormatOptions` — unit or formatting options (optional)
...
**Words format:** `one|two|few|many|other|zero` (order matters!)
...
intl.plural(0, 'item|items') // '0 items'

const intlRU = new GeoIntl('ru-RU')
// Russian plural forms (more complex)
intlRU.plural(1, 'товар|товара|товаров') // '1 товар'
intlRU.plural(2, 'товар|товара|товаров') // '2 товара'
...
Returns list of all months in dropdown format.
...
// 10 day limit
intl.relativeLimit(weekAgo, 10) // '1 week ago'
...
```javascript
const names = ['Zebra', 'Apple', 'Ñoño', 'Beta']
intl.sort(names) // ['Apple', 'Beta', 'Ñoño', 'Zebra'] (correct order for English)

// With custom comparison function
const users = [{ name: 'Zebra' }, { name: 'Apple' }]
