...
// Вход
const loginResult = auth.login({ username: 'ivan', password: 'secret' })
console.log('Вход успешен:', loginResult.user.name)

// Проверка аутентификации
console.log('Аутентифицирован:', auth.isAuthenticated()) // true
console.log('Текущий пользователь:', auth.getCurrentUser().name)

// Выход
auth.logout()
console.log('Аутентифицирован после выхода:', auth.isAuthenticated()) // false
```

### Постоянство корзины покупок
...
class ShoppingCart {
  ...
  getItems() {
    return this.cartCookie.get([]) // По умолчанию пустой массив
  }
...
// Добавить продукты
cart.addItem({ id: 1, name: 'Ноутбук', price: 79999.99 }, 1)
cart.addItem({ id: 2, name: 'Мышь', price: 2499.99 }, 2)

console.log('Элементы корзины:', cart.getItems())
console.log('Общая цена:', cart.getTotalPrice())
console.log('Количество элементов:', cart.getItemCount())

// Обновить количества
cart.updateQuantity(2, 1) // Уменьшить количество мышей до 1
cart.removeItem(1) // Удалить ноутбук

console.log('Обновлённая корзина:', cart.getItems())
```

## Интеграция с CookieBlock

Класс Cookie автоматически учитывает настройки CookieBlock для обеспечения соблюдения приватности:

```javascript
import { CookieBlock } from '@dxtmisha/functional'

// Заблокировать cookie (имитация выбора пользователя по приватности)
CookieBlock.set(true)

const marketingCookie = new Cookie('marketing-data')

// Это фактически не установит cookie, потому что CookieBlock активен
...
// Теперь cookie работают нормально
marketingCookie.set('marketing-value')
console.log('Marketing cookie:', marketingCookie.get()) // 'marketing-value'
```

Класс Cookie предоставляет мощный, соответствующий приватности способ управления браузерными cookie с автоматической сериализацией и гибкими опциями конфигурации, делая его идеальным для современных веб-приложений, которые должны учитывать пользовательские настройки приватности.
