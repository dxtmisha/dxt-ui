...
Reactive states:
- `data: ComputedRef<ApiData<R> | undefined>` — A reference to the loaded data (Computed). Implements **lazy loading**: the server request is automatically initiated on the first access to this property.
- `item: Ref<ApiData<R> | undefined>` — Direct reference to data (Ref). Also initiates **lazy loading** upon access.
...
- `init(): void` — Method for explicit request initialization. Triggered automatically on first access to `data`, but can be called manually.
