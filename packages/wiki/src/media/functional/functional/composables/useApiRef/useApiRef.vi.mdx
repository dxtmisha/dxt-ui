...
Trạng thái phản ứng:
- `data: ComputedRef<ApiData<R> | undefined>` — Tham chiếu đến dữ liệu đã tải (Computed). Thực hiện **lazy loading**: yêu cầu máy chủ được tự động bắt đầu khi truy cập lần đầu vào thuộc tính này.
- `item: Ref<ApiData<R> | undefined>` — Tham chiếu trực tiếp đến dữ liệu (Ref). Cũng bắt đầu **lazy loading** khi truy cập.
...
- `init(): void` — Phương thức để khởi tạo yêu cầu một cách rõ ràng. Tự động kích hoạt khi truy cập `data` lần đầu, nhưng có thể được gọi thủ công.
