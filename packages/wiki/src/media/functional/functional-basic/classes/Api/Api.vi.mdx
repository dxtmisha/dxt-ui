...
Kiểm tra xem mã có đang thực thi trên một máy chủ phát triển localhost không (trả về `true` nếu `location.hostname === 'localhost'`).

**Trả về:** `boolean`

### `getStatus`

Trả về trình quản lý trạng thái `ApiStatus` để theo dõi các tác vụ, trạng thái, lỗi, và các đối tượng mã hóa gói đầy đủ phản hồi từ máy chủ cuối cùng.

**Trả về:** `ApiStatus`

### `getResponse`

Trả về trình quản lý bộ giả lập và phản hồi `ApiResponse` phục vụ cấu hình Mock data (khi đang phát triển offline hoặc giao diện đang làm chưa có API đi kèm).

**Trả về:** `ApiResponse`

## Form gửi thông tin (Request Preparation)

### `getUrl`

Tạo ra đường dẫn API đầy đủ, tích hợp và xử lý với các biến đo lường địa lý được cấu hình: `{locale}`, `{country}`, `{language}`.

**Tham số:**
- `path: string` — đường dẫn chỏ đến hệ thống.
- `api?: boolean` — (Mặc định: `true`) Cho biết có thêm phần cơ sở vào API không.

**Trả về:** `string` — chuỗi URL đích trọn vẹn.

### `getBody`

Hoán đổi tài liệu để nạp theo yêu cầu (nguồn gốc) thực thể (không dùng được trong hàm `GET`). Có khả năng kết xuất và xử lý dữ liệu ở cả lớp biến chuỗi JSON nguyên gốc tĩnh để có kết cấu `FormData`.

**Tham số:**
- `request?: ApiFetch['request']` — bộ tham chiếu yêu cầu của thao tác (fetch).
- `method?: string` — phương thức lệnh HTTP hiện tại (đây là cài đặt mặc định để thành `ApiMethodItem.get`).

**Trả về:** `string | FormData | undefined`

### `getBodyForGet`

Hoàn trả dữ liệu query, thường được sử dụng và tạo dựng parameter khi cài một thao tác kiểu `GET`.

**Tham số:**
- `request: ApiFetch['request']` — khối query object.
- `path?: string` — một lộ trình dẫn ban đầu.
- `method?: string` — biểu thức HTTP tương ứng.

**Trả về:** `string` — hệ nội dung tìm kiếm đã được chuyển đổi phù hợp để nối vào URL (`?key=value&key2=val2`).

## Các phương thức cấu hình (Configuration Methods)

Toàn bộ sửa đổi hoặc cấu hình hệ thống hiện có thể sử dụng phương pháp nối chuỗi (chaining methods) ngay tức thì trên cùng một bản thể (instances mapping).

### `setHeaders`

Áp dụng một thư mục Header chuẩn cho toàn hệ thống (như Token nhận diện định danh) nhúng vào hệ thống mỗi khi gửi yêu cầu chuyển qua lại.

**Tham số:**
- `headers: Record<string, string>` — Khung biểu đồ dạng key:value nguyên thể.

**Trả về:** `Api`

### `setRequestDefault`

Đưa toàn bộ thông số tham số truyền thông vào khung chuyển tín đồ lưu thông trên máy chủ từ xa.

**Tham số:**
- `request: Record<string, any>` — Các phép lệnh chèn nội dung quy ước mặc định.

**Trả về:** `Api`

### `setUrl`

Ghi đè định danh URL trên nền tảng (cài đặt mặc định đánh giá ở nhánh \`/api/\`).

**Tham số:**
- `url: string` — Tiền tố môi trường kết nối bao trùm chuỗi kết nối schema.

**Trả về:** `Api`

### `setPreparation`

Tạo mới hook tùy chỉnh cài đặt hoạt động chạy liên tục _ngay trước_ thời khắc thao tác bắt đầu; chức năng hữu hiệu lớn nằm trong tiền xử lý xuất màn hình mờ/loader chung hoặc sửa đổi tham số gốc.

**Tham số:**
- `callback: (apiFetch: ApiFetch) => Promise<void>` — khối trigger đồng bộ. Nó nhận tham chiếu trực tiếp đến file tùy chọn cài đặt của phiên yêu cầu hiện tại (request config).

**Trả về:** `Api`

### `setEnd`

Tạo mới hook cài đặt _ngay sau_ một pha trao đổi mạng. Phản hồi kèm `{ reset: true }` cho phép ngắt các tiến trình phiên mã rồi tự tái bắt vòng chu trình.

**Tham số:**
- `callback: (query: Response, apiFetch: ApiFetch) => Promise<ApiPreparationEnd>` — vòng xử lý sự kiện phía bên kia server phản hồi lại. Hàm sẽ nhận nguyên mẫu cả dữ liệu phản hồi lẫn bộ thông số cài đặt đầu vào.
...
## Phương thức điều hành tín hiệu (API Calling Methods)
