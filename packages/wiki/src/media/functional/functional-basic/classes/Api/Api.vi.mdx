...
Các phương thức trả về lớp `Api` để cho phép tạo chuỗi lệnh.

- `setUrl(url: string): Api` — Thay đổi URL base của API.
- `setHeaders(headers: Record<string, string>): Api` — Thiết lập các header yêu cầu mặc định.
- `setRequestDefault(request: Record<string, any>): Api` — Thiết lập dữ liệu body yêu cầu mặc định.
- `setPreparation(callback): Api` — Đăng ký một hook để chạy trước mỗi yêu cầu được bật.
- `setEnd(callback): Api` — Đăng ký một hook để chạy sau mỗi phản hồi được bật.
- `setTimeout(timeout: number): Api` — Thay đổi timeout yêu cầu toàn cục.
- `setConfig(config?: ApiConfig): Api` — Áp dụng một đối tượng cấu hình đầy đủ.
