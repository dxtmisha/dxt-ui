...
- **Locale awareness** — automatically uses the current geographic locale
...
- **List operations** — fetch multiple translations in a single call
...
Synchronously retrieves multiple translations from a cache.
...
Adds translation codes to the loading queue for batch fetching.
...
Translate.add('text3') // All sent in a single request
```

### `addSync`

Directly adds translations to the cache (for development/testing).
...
Intelligently adds translations based on the environment (API call vs. direct).
...
```javascript
// Automatically chooses a method based on Api.isLocalhost()
...
Synchronously adds translations grouped by location (locale) directly to the cache.

**Parameters:**
- `data: Record<string, Record<string, string>>` — list of texts by location

```javascript
// Add translations for different locations
...
class MyComponent {
  async loadTexts() {
    // Add necessary translations to the queue
    await Translate.add([
      'component.title',
