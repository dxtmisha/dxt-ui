...
- `get<T>(key: string): Promise<T | undefined>` — Truy xuất một giá trị từ bộ nhớ đệm.
- `getByFetch<T>(fetch: ApiFetch): Promise<T | undefined>` — Truy xuất giá trị bằng cách sử dụng các tùy chọn yêu cầu làm nguồn tạo khóa.
- `set<T>(key: string, value: T, age?: number): Promise<void>` — Lưu trữ một giá trị vào bộ nhớ đệm.
- `setByFetch<T>(fetch: ApiFetch, value: T): Promise<void>` — Lưu trữ giá trị dựa trên các tùy chọn yêu cầu.
- `remove(key: string): Promise<void>` — Xóa thủ công một bản ghi.

## Cấu hình

- `init(getListener, setListener, removeListener, cacheStepAgeClearOld?): void` — Cấu hình bộ nhớ đệm với các bộ lắng nghe lưu trữ.
- `reset(): typeof ApiCache` — Xóa các mục trong bộ nhớ và ngắt kết nối các bộ lắng nghe.
