...
- `wrapper?: <R>(callback: () => Promise<R>, apiFetch: ApiFetch) => Promise<R>` — hàm wrapper để thực thi các yêu cầu.
...
- `setWrapper(wrapper: <R>(callback: () => Promise<R>, apiFetch: ApiFetch) => Promise<R>): this` — Thiết lập hàm wrapper cho các yêu cầu.
...
- `wrapper?: <R>(callback: () => Promise<R>, apiFetch: ApiFetch) => Promise<R>` — hàm wrapper cục bộ để thực thi yêu cầu cụ thể này.
...
## Hàm bọc yêu cầu (Request Wrapper)

Hàm bọc yêu cầu (`wrapper`) cho phép bạn chặn và bọc quá trình thực thi của một yêu cầu mạng vật lý bằng một hàm tùy chỉnh. Điều này đặc biệt hữu ích khi tích hợp với các công cụ Giám sát hiệu suất ứng dụng (APM) như Sentry hoặc OpenTelemetry để cấu hình và theo dõi thời gian thực thi yêu cầu (tracing).

Hàm bọc có thể được cấu hình toàn cục cho toàn bộ instance (thông qua `options.wrapper` hoặc phương thức `setWrapper`) hoặc cục bộ cho một yêu cầu duy nhất bên trong đối tượng `ApiFetch`.

### Ví dụ sử dụng với Sentry:
