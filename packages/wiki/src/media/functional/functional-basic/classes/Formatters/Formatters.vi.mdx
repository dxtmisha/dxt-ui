...
Tạo một thực thể `Formatters` mới với các tùy chọn định dạng được chỉ định và một danh sách khởi tạo tùy chọn.
...
const options = {
  ...
  status: {
    transformation: (value) => value === 1 ? 'Đang hoạt động' : 'Ngừng hoạt động'
  }
}
...
Phương thức cốt lõi xử lý danh sách và trả về một mảng mới, nơi mỗi mục được bổ sung các giá trị đã định dạng dựa trên cấu hình ban đầu.

**Trả về:** `FormattersListColumns<Item, Options>` — một mảng đối tượng mới với các thuộc tính `*Format` bổ sung.
...
- `getList(): List | undefined` — Trả về danh sách dữ liệu hiện tại.
- `getOptions(): Options` — Trả về cấu hình định dạng hiện tại.
- `setList(list: List): this` — Cập nhật danh sách dữ liệu cần định dạng và trả về thực thể để gọi chuỗi lệnh.
