...
Создает новый экземпляр `Formatters` с указанными параметрами форматирования и необязательным начальным списком.
...
const options = {
  ...
  status: {
    transformation: (value) => value === 1 ? 'Активен' : 'Неактивен'
  }
}
