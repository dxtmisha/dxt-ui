...
- **Hướng đối tượng** — cho phép tạo nhiều phiên bản (instances) với các cấu hình khác nhau.
...
- `getFirstDay(): string` — Trả về mã cho ngày đầu tiên của tuần của địa phương hiện tại.
...
- `getByCode(code?: string): GeoItemFull` — Tìm kiếm dữ liệu theo mã ngôn ngữ hoặc quốc gia (thông tin cơ bản).
- `getByCodeFull(code: string): GeoItem | undefined` — Tìm kiếm kết quả khớp chính xác theo mã địa phương đầy đủ.
- `getByCountry(country: string): GeoItem | undefined` — Tìm kiếm dữ liệu theo mã quốc gia.
