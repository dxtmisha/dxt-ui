...
Triggers error handling. It looks up the pre-registered data for the provided error code, merges it with the input `cause` based on priority, and passes it to the internal handler. By default, the input `cause` overrides stored data, but you can use the `priority` field to protect specific values.
