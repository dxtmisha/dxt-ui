...
Về cơ chế chạy ở bên trong của quá trình phân giải, toàn bộ module `Cache` tạo và trượt dữ liệu của nó ở từng dòng biến trên những phần tử gắn riêng biệt bằng list `CacheItem`. Logic tạo rào lưu ở `CacheItem` quản lý biến (value) thời điểm truy cập, giá trị thời gian biến cũ và định nghĩa tính hợp lệ logic (`isUpdate`).
