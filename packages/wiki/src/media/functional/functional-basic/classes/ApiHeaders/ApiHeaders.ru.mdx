...
Создает новый экземпляр `ApiHeaders`. Класс используется для хранения и управления заголовками, которые будут автоматически добавляться к каждому HTTP-запросу. Обеспечивает централизованное управление такими данными, как токены авторизации или метаданные приложения.
