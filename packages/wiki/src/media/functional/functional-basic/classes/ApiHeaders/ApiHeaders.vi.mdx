...
Tạo một thực thể `ApiHeaders` mới. Lớp này được sử dụng để lưu trữ và quản lý các tiêu đề (headers) sẽ được tự động thêm vào mỗi yêu cầu HTTP. Cung cấp khả năng quản lý tập trung cho các dữ liệu như mã thông báo xác thực (authorization tokens) hoặc siêu dữ liệu ứng dụng.
