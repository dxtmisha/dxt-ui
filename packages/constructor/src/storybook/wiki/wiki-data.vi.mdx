...
<Meta title='@dxtmisha/vi/constructor/4. Cấu hình dữ liệu Wiki'/>
...
- `options: StorybookArgsToOptionsByType` — đối tượng cho các cài đặt hành vi và hiển thị nâng cao.
- `isDemo?: boolean` — chỉ ra rằng tham số này chỉ dành cho mục đích demo.
- `demo?: any` — dữ liệu demo sẽ được thay thế trong các ví dụ.
- `demoOptions?: Record<string, any>` — các tùy chọn demo bổ sung cho phần tử.
...
- `type?: StorybookControl | string | string[]` — kiểu dữ liệu hoặc kiểu điều khiển đích (ví dụ: `'string'`, `['string', 'File']`).
...
- `description?: StorybookArgsToDescription` (`string | Record<string, string>`) — mô tả của thuộc tính. Có thể là một chuỗi hoặc một đối tượng có bản dịch theo ngôn ngữ (`en`, `ru`, `vi`).
- `options?: (string | number)[] | Record<string, string>` — danh sách các tùy chọn có sẵn để lựa chọn (có liên quan đến kiểu `select`).
...
- `text` — trường văn bản.
- `string` — trường nhập chuỗi ký tự.
- `number` — nhập số.
- `boolean` — hộp kiểm (true/false).
- `select` — danh sách thả xuống (yêu cầu điền `options`).
- `array` — nhập mảng giá trị.
- `object` — trình chỉnh sửa cho các đối tượng JSON.

#### `StorybookCategory`
Được sử dụng để nhóm các thuộc tính vào các khối logic:
- `actions` (`'Actions'`) — hành động và sự kiện.
- `adaptive` (`'Adaptive'`) — các tham số về khả năng đáp ứng và căn chỉnh thích ứng.
- `aria` (`'ARIA'`) — các thuộc tính trợ năng (ARIA).
- `arrow` (`'Arrow'`) — các tham số mũi tên và chỉ hướng.
- `attributes` (`'Attributes'`) — các thuộc tính HTML và thuộc tính hệ thống.
- `bars` (`'Bars'`) — các tham số thanh, bảng điều khiển và chỉ báo tiến trình.
- `counter` (`'Counter'`) — các tham số bộ đếm và huy hiệu.
- `form` (`'Form'`) — các thuộc tính biểu mẫu và phần tử nhập liệu.
- `hook` (`'Hook'`) — các tham số hook và hàm composable.
- `icon` (`'Icon'`) — các tham số biểu tượng (icon).
- `information` (`'Information'`) — các thuộc tính thông tin và chú thích.
- `mask` (`'Mask'`) — các tham số mặt nạ nhập liệu.
- `option` (`'Options'`) — các tùy chọn và danh sách lựa chọn.
- `search` (`'Search'`) — các tham số tìm kiếm và bộ lọc.
- `sort` (`'Sort'`) — các tham số sắp xếp.
- `status` (`'Status'`) — các trạng thái thành phần (đang tải, lỗi, hoạt động, v.v.).
- `style` (`'Styles'`) — các tham số ngoại hình và phong cách.
- `technical` (`'Technical'`) — các cài đặt kỹ thuật và nội bộ.
- `text` (`'Text'`) — các tham số văn bản và kiểu chữ.
- `value` (`'Values'`) — các giá trị chính của thành phần.
