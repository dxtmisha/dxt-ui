import { i as e } from './preload-helper-Cv19cBWa.js'; import { a as t, t as n } from './jsx-runtime-D1H6-shM.js'; import { f as r, s as i } from './blocks-CKlddtvc.js'; import { t as a } from './mdx-react-shim-B1U8VZv0.js'; function o(e) {
  let n = { code: `code`, h1: `h1`, p: `p`, pre: `pre`, strong: `strong`, ...t(), ...e.components }; return (0, c.jsxs)(c.Fragment, { children: [(0, c.jsx)(i, { title: `@dxtmisha/vi/# UI/properties/flags/Chủ đề (theme)` }), `
`, (0, c.jsx)(n.h1, { id: `chủ-đề-theme`, children: `Chủ đề (theme)` }), `
`, (0, c.jsxs)(n.p, { children: [`Cờ `, (0, c.jsx)(n.code, { children: `_theme` }), ` cho phép giới hạn phạm vi của các kiểu cho một chủ đề màu cụ thể. Các kiểu sẽ chỉ được tạo bên trong bộ chọn của chủ đề tương ứng.`] }), `
`, (0, c.jsx)(n.p, { children: (0, c.jsxs)(n.strong, { children: [`Cấu hình đầu vào (`, (0, c.jsx)(n.code, { children: `properties.json` }), `):`] }) }), `
`, (0, c.jsx)(n.pre, { children: (0, c.jsx)(n.code, { className: `language-json`, children: `{
  "button": {
    "background": {
      "value": "#000",
      "_theme": "dark"
    }
  }
}
` }) }), `
`, (0, c.jsx)(n.p, { children: (0, c.jsx)(n.strong, { children: `Kết quả (SCSS):` }) }), `
`, (0, c.jsx)(n.pre, { children: (0, c.jsx)(n.code, { className: `language-scss`, children: `.theme--dark {
  .button {
    background-color: #000;
  }
}
` }) })] })
} function s(e = {}) { let { wrapper: n } = { ...t(), ...e.components }; return n ? (0, c.jsx)(n, { ...e, children: (0, c.jsx)(o, { ...e }) }) : o(e) } var c; e(() => { c = n(), a(), r() })(); export { s as default }
