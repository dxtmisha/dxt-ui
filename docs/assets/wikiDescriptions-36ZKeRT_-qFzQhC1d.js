...
Controlled primarily through the label, icon, and dot props. Use for unread counts, online status indicators, or marking items as "New" or "Sale" in e-commerce contexts.
    `}},{name:`Bleed`,description:{en:`A component that allows content to expand beyond the horizontal boundaries of its parent container`,ru:`Компонент, позволяющий контенту выходить за горизонтальные границы родительского контейнера`},possibilities:{en:[`automatic calculation of negative margins`,`flexible tag selection`,`useful for full-width images or blocks in padded containers`],ru:[`автоматический расчет отрицательных отступов`,`гибкий выбор HTML-тега`,`полезно для изображений или блоков во всю ширину в контейнерах с отступами`]},import:[],render:`
      <DesignComponent v-bind="args">
...
Highly customizable through design tokens and adaptive layout options. Controlled primarily via click events and various styling props. Use for call-to-actions, navigation links, or icon-only toggle buttons.
    `}},{name:`ButtonGroup`,description:{en:`A container component for grouping multiple buttons together.`,ru:`Компонент-контейнер для группировки нескольких кнопок вместе.`},possibilities:{en:[`horizontal and vertical orientations`,`automatic wrapping for horizontal layout`,`passing shared attributes to all buttons`,`flexible content via list prop or default slot`,`consistent spacing between buttons`],ru:[`горизонтальная и вертикальная ориентация`,`автоматический перенос для горизонтального расположения`,`передача общих атрибутов всем кнопкам`,`гибкий контент через свойство list или слот по умолчанию`,`согласованные отступы между кнопками`]},import:[],stories:[{id:`ButtonGroupSlots`,name:{en:`Slots usage`,ru:`Использование слотов`},template:`
        <DesignComponent>
          <template #default>
            Default Slot
          </template>
...
Controlled via v-model:open and various content props (label, description). Use for critical operations requiring user confirmation, operation success/fail alerts, or informative system messages.
    `}},{name:`Divider`,description:{en:`A component for visual separation of content blocks`,ru:`Компонент для визуального разделения блоков контента`},possibilities:{en:[`customizable line width and color via tokens`,`clean and minimal design`,`used to improve visual hierarchy`],ru:[`настраиваемая ширина и цвет линии через токены`,`чистый и минималистичный дизайн`,`используется для улучшения визуальной иерархии`]},import:[],stories:[],documentation:{body:`
...
Controlled via v-model and type props, emitting standard input/change events. Use for all text-based data collection, from simple names to complex formatted financial or identification data.
    `}},{name:`InputPhoneDialCode`,description:{en:`Trigger button for selecting a country phone dial code via a country menu`,ru:`Кнопка-триггер для выбора кода страны телефонного номера через меню стран`},possibilities:{en:[`automatic detection of the current country via Geo.getCountry()`,`country selection via a MenuCountry popup with flag and phone code display`,`configurable label display: country name, phone code, or none`,`reactive model binding via v-model:selected`,`customizable trailing icon (arrow) for open/close indication`,`inherits all Button and MenuCountry capabilities`],ru:[`автоматическое определение текущей страны через Geo.getCountry()`,`выбор страны через всплывающее меню MenuCountry с флагом и кодом телефона`,`настраиваемый тип метки: название страны, телефонный код или без метки`,`реактивная привязка модели через v-model:selected`,`настраиваемая иконка-стрелка для индикации открытия/закрытия`,`наследует все возможности компонентов Button и MenuCountry`]},import:[`import { ref } from 'vue'`],stories:[{id:`InputPhoneDialCodeVModel`,name:{en:`Two-way binding (v-model)`,ru:`Двусторонняя привязка (v-model)`},setup:`
      return {
...
Controlled via the list prop for items and v-model:selected for state tracking. Use for user profile menus, sidebar navigation, or advanced select-like UI patterns where standard dropdowns lack sufficient structural flexibility.
    `}},{name:`MenuButton`,description:{en:`Button-based dropdown selector combining Button, Menu, and Select for clean, compact selection interfaces`,ru:`Кнопочный выпадающий селектор, объединяющий Button, Menu и Select для компактных интерфейсов выбора`},possibilities:{en:[`button trigger interface instead of standard field outline`,`displays selected value names with a custom separator`,`supports hiding the value when an icon is present (hideValueIcon)`,`supports hiding the label when a value is selected (hideLabelValue)`,`built-in search and filtering inside the dropdown header`,`single or multiple value selection`,`standard form data integration via a hidden input element`],ru:[`интерфейс кнопки-триггера вместо стандартной рамки поля`,`отображает имена выбранных значений с настраиваемым разделителем`,`поддержка скрытия значения при наличии иконки (hideValueIcon)`,`поддержка скрытия метки при выборе значения (hideLabelValue)`,`встроенный поиск и фильтрация внутри заголовка выпадающего меню`,`одиночный или множественный выбор значений`,`стандартная интеграция данных формы через скрытый элемент ввода input`]},import:[`import { ref } from 'vue'`],stories:[{id:`MenuButtonBasic`,name:{en:`Basic`,ru:`Базовые`},setup:`
      const options = ref([
      ...
      `},{id:`MenuButtonVModel`,name:{en:`Two-way binding (v-model)`,ru:`Двусторонняя привязка (v-model)`},setup:`
      ...
      `},{id:`MenuButtonSkeleton`,name:{en:`Skeleton`,ru:`Скелетон`},components:[`Skeleton`],template:`
        <DesignSkeleton :active="true" style="max-width:320px">
          <div class="wiki-storybook-flex-column">
            <DesignComponent
              type="menuButton"
...
Integrated hidden inputs provide standard form data carrying, while optional header search components (showSearch) allow filterable options lists.
    `}},{name:`MenuChip`,description:{en:`Chip-based dropdown selector combining Chip, Menu, and Select for clean, compact selection interfaces`,ru:`Выпадающий селектор на основе чипа (Chip), объединяющий Chip, Menu и Select для компактных интерфейсов выбора`},possibilities:{en:[`chip trigger interface instead of standard field outline`,`displays selected value names with a custom separator`,`supports hiding the value when an icon is present (hideValueIcon)`,`supports hiding the label when a value is selected (hideLabelValue)`,`built-in search and filtering inside the dropdown header`,`single or multiple value selection`,`standard form data integration via a hidden input element`],ru:[`интерфейс чипа-триггера вместо стандартной рамки поля`,`отображает имена выбранных значений с настраиваемым разделителем`,`поддержка скрытия значения при наличии иконки (hideValueIcon)`,`поддержка скрытия метки при выборе значения (hideLabelValue)`,`встроенный поиск и фильтрация внутри заголовка выпадающего меню`,`одиночный или множественный выбор значений`,`стандартная интеграция данных формы через скрытый элемент ввода input`]},import:[`import { ref } from 'vue'`],stories:[{id:`MenuChipBasic`,name:{en:`Basic`,ru:`Базовые`},setup:`
      const options = ref([
        { label: 'English', value: 'en' },
        { label: 'Russian', value: 'ru' },
        { label: 'Vietnamese', value: 'vi' },
      ...
      `},{id:`MenuChipVModel`,name:{en:`Two-way binding (v-model)`,ru:`Двусторонняя привязка (v-model)`},setup:`
      const valueSingle = ref('option2')
      const valueMultiple = ref(['option1', 'option3'])
      const options = [
        { label: 'First option', value: 'option1' },
      ...
      `},{id:`MenuChipSkeleton`,name:{en:`Skeleton`,ru:`Скелетон`},components:[`Skeleton`],template:`
        <DesignSkeleton :active="true" style="max-width:320px">
          <div class="wiki-storybook-flex-column">
            <DesignComponent
              type="menuChip"
...
Displays selected values with configurable label separators (labelSeparator) and label hiding options (hideLabelValue, hideValueIcon).
Integrated hidden inputs provide standard form data carrying, while optional header search components (showSearch) allow filterable options lists.
    `}},{name:`MenuCountry`,description:{en:`Specialized menu for selecting countries with automatic flag and name loading`,ru:`Специализированное меню для выбора стран с автоматической загрузкой флагов и названий`},possibilities:{en:[`automatic generation of country list via GeoFlagRef`,`filtering by country codes via countryList prop`,`all features of the base Menu component`,`support for internationalized country names`],ru:[`автоматическая генерация списка стран через GeoFlagRef`,`фильтрация по кодам стран через свойство countryList`,`все возможности базового компонента Menu`,`поддержка интернационализированных названий стран`]},import:[`import { ref } from 'vue'`],render:`
    <DesignComponent v-bind="args">
      <template #control="{binds}">
        <button class="wiki-storybook-button" v-bind="binds">Select Country</button>
      </template>
    </DesignComponent>
  `,stories:[{id:`MenuCountryFilter`,name:{en:`Filtering countries`,ru:`Фильтрация стран`},setup:`
      return {
...
Controlled via the visible, value, and indeterminate props, with support for custom min/max bounds. Use for page-level loading bars, button action feedback, or inline file upload progress.
    `}},{name:`Radio`,description:{en:`Interactive radio button component for single choice selection from a group`,ru:`Интерактивный компонент радиокнопки для выбора одного варианта из группы`},possibilities:{en:[`single selection within a group`,`built-in validation support with custom messages`,`label and description text support`,`customizable icons for selected state`,`disabled and loading states`,`ripple effect on interaction`,`skeleton state for loading placeholders`,`adaptive layout options`,`form integration with name and value attributes`],ru:[`выбор одного значения из группы`,`встроенная поддержка валидации с пользовательскими сообщениями`,`поддержка текста метки и описания`,`настраиваемые иконки для выбранного состояния`,`состояния отключения и загрузки`,`эффект волны при взаимодействии`,`состояние скелетона для заполнителей загрузки`,`опции адаптивной разметки`,`интеграция с формами через атрибуты name и value`]},import:[`import { ref } from 'vue'`],stories:[{id:`RadioVModel`,name:{en:`Two-way binding (v-model)`,ru:`Двусторонняя привязка (v-model)`},setup:`
      return {
      ...
      `},{id:`RadioSkeleton`,name:{en:`Skeleton`,ru:`Скелетон`},components:[`Skeleton`],template:`
        <DesignSkeleton :active="true">
          <div class="wiki-storybook-flex-column">
      ...
      `},{id:`RadioSlots`,name:{en:`Slots usage`,ru:`Использование слотов`},template:`
        <DesignComponent>
          <template #default>
...
Controlled via simple content props or an actions-list array for complex button configurations. Use as the base message unit for all system feedback, ensuring high visibility and accessibility through automatic ARIA status role implementation.
    `}},{name:`Switch`,description:{en:`Interactive toggle switch component for binary on/off state selection`,ru:`Интерактивный переключатель (тогл) для выбора бинарного состояния «включено»/«выключено»`},possibilities:{en:[`binary toggling states (on / off)`,`built-in validation support with custom messages`,`label and description text support`,`customizable track and slider elements`,`disabled and loading states`,`ripple effect on interaction`,`skeleton state for loading placeholders`,`adaptive layout options`,`form integration with name and value attributes`],ru:[`бинарные переключаемые состояния (вкл / выкл)`,`встроенная поддержка валидации с пользовательскими сообщениями`,`поддержка текста метки и описания`,`настраиваемые элементы трека и ползунка`,`состояния отключения и загрузки`,`эффект волны при взаимодействии`,`состояние скелетона для заполнителей загрузки`,`опции адаптивной разметки`,`интеграция с формами через атрибуты name и value`]},import:[`import { ref } from 'vue'`],stories:[{id:`SwitchVModel`,name:{en:`Two-way binding (v-model)`,ru:`Двусторонняя привязка (v-model)`},setup:`
      return {
      ...
      `},{id:`SwitchSkeleton`,name:{en:`Skeleton`,ru:`Скелетон`},components:[`Skeleton`],template:`
        <DesignSkeleton :active="true">
          <div class="wiki-storybook-flex-column">
      ...
      `},{id:`SwitchSlots`,name:{en:`Slots usage`,ru:`Использование слотов`},template:`
        <DesignComponent>
          <template #default>
            <strong>Custom label slot</strong>
          </template>
...
Primarily integrated as a subcomponent; should generally not be manually used for standalone input fields. Controlled via reactive value bindings, serving as the high-performance core for any dynamic data entry element requiring responsive layout scaling.
    `,hide:!0}},{name:`TextDescription`,description:{en:`A simple component for displaying text description with dynamic HTML tag support`,ru:`Простой компонент для отображения текстового описания с поддержкой динамического HTML-тега`},possibilities:{en:[`renders text from description property or description slot`,`supports dynamic HTML tags (span, div, p, etc.)`,`fully compatible with standard design tokens`,`clean and lightweight DOM output`],ru:[`выводит текст из свойства description или слота описания`,`поддерживает динамические HTML-теги (span, div, p и т.д.)`,`полностью совместим со стандартными токенами дизайна`,`чистый и легковесный DOM`]},import:[],stories:[],documentation:{body:`
...
A utility component for displaying textual descriptions. Supports description slot injection, custom HTML tags via "tag" property (defaults to "span"), and clean semantic output. Integrated with DescriptionInclude for standardized description rendering.
    `}},{name:`TextLabel`,description:{en:`A simple component for displaying text or labels with dynamic HTML tag support`,ru:`Простой компонент для отображения текста или меток с поддержкой динамического HTML-тега`},possibilities:{en:[`renders text from label property or default slot`,`supports dynamic HTML tags (span, div, p, h1-h6, etc.)`,`fully compatible with standard design tokens`,`clean and lightweight DOM output`],ru:[`выводит текст из свойства label или слота по умолчанию`,`поддерживает динамические HTML-теги (span, div, p, h1-h6 и т.д.)`,`полностью совместим со стандартными токенами дизайна`,`чистый и легковесный DOM`]},import:[],stories:[],documentation:{body:`
