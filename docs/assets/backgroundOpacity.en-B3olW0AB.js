import { i as e } from './preload-helper-Cv19cBWa.js'; import { a as t, t as n } from './jsx-runtime-D1H6-shM.js'; import { f as r, s as i } from './blocks-CKlddtvc.js'; import { t as a } from './mdx-react-shim-B1U8VZv0.js'; function o(e) {
  let n = { code: `code`, h1: `h1`, h2: `h2`, li: `li`, p: `p`, pre: `pre`, strong: `strong`, ul: `ul`, ...t(), ...e.components }; return (0, c.jsxs)(c.Fragment, { children: [(0, c.jsx)(i, { title: `@dxtmisha/en/styles/color/backgroundOpacity - Set background opacity` }), `
`, (0, c.jsx)(n.h1, { id: `backgroundopacity`, children: (0, c.jsx)(n.code, { children: `backgroundOpacity` }) }), `
`, (0, c.jsxs)(n.p, { children: [`The mixin sets the `, (0, c.jsx)(n.code, { children: `--sys-background-opacity` }), ` variable, allowing for independent control of background transparency.`] }), `
`, (0, c.jsx)(n.p, { children: (0, c.jsx)(n.strong, { children: `Parameters:` }) }), `
`, (0, c.jsxs)(n.ul, { children: [`
`, (0, c.jsxs)(n.li, { children: [(0, c.jsx)(n.code, { children: `$opacity: Number | String` }), ` — transparency value [0..1] or CSS variable.`] }), `
`, (0, c.jsxs)(n.li, { children: [(0, c.jsx)(n.code, { children: `$important: Boolean` }), ` [false] — if `, (0, c.jsx)(n.code, { children: `true` }), `, adds `, (0, c.jsx)(n.code, { children: `!important` }), `.`] }), `
`, (0, c.jsxs)(n.li, { children: [(0, c.jsx)(n.code, { children: `$init: Boolean` }), ` [false] — if `, (0, c.jsx)(n.code, { children: `true` }), `, immediately applies the `, (0, c.jsx)(n.code, { children: `background-color` }), ` property.`] }), `
`] }), `
`, (0, c.jsx)(n.h2, { id: `example`, children: `Example` }), `
`, (0, c.jsx)(n.pre, { children: (0, c.jsx)(n.code, { className: `language-scss`, children: `@use '@dxtmisha/styles';

.bg-faded {
  @include styles.backgroundOpacity(0.5);
}

.bg-faded-important {
  @include styles.backgroundOpacity(var(--my-opacity), true, true);
}
` }) })] })
} function s(e = {}) { let { wrapper: n } = { ...t(), ...e.components }; return n ? (0, c.jsx)(n, { ...e, children: (0, c.jsx)(o, { ...e }) }) : o(e) } var c; e(() => { c = n(), a(), r() })(); export { s as default }
