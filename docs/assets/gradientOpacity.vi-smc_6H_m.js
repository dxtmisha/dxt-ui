import { i as e } from './preload-helper-Cv19cBWa.js'; import { a as t, t as n } from './jsx-runtime-D1H6-shM.js'; import { f as r, s as i } from './blocks-CKlddtvc.js'; import { t as a } from './mdx-react-shim-B1U8VZv0.js'; function o(e) {
  let n = { code: `code`, h1: `h1`, h2: `h2`, li: `li`, p: `p`, pre: `pre`, strong: `strong`, ul: `ul`, ...t(), ...e.components }; return (0, c.jsxs)(c.Fragment, { children: [(0, c.jsx)(i, { title: `@dxtmisha/vi/styles/color/gradientOpacity - Độ mờ gradient` }), `
`, (0, c.jsx)(n.h1, { id: `gradientopacity`, children: (0, c.jsx)(n.code, { children: `gradientOpacity` }) }), `
`, (0, c.jsxs)(n.p, { children: [`Mixin này thiết lập biến `, (0, c.jsx)(n.code, { children: `--sys-gradient-opacity` }), `, cho phép kiểm soát tính trong suốt của dải màu nền (gradient) một cách độc lập.`] }), `
`, (0, c.jsx)(n.p, { children: (0, c.jsx)(n.strong, { children: `Tham số:` }) }), `
`, (0, c.jsxs)(n.ul, { children: [`
`, (0, c.jsxs)(n.li, { children: [(0, c.jsx)(n.code, { children: `$opacity: Number | String` }), ` — giá trị trong suốt [0..1] hoặc biến CSS.`] }), `
`, (0, c.jsxs)(n.li, { children: [(0, c.jsx)(n.code, { children: `$important: Boolean` }), ` [false] — nếu `, (0, c.jsx)(n.code, { children: `true` }), `, thêm `, (0, c.jsx)(n.code, { children: `!important` }), `.`] }), `
`, (0, c.jsxs)(n.li, { children: [(0, c.jsx)(n.code, { children: `$init: Boolean` }), ` [false] — nếu `, (0, c.jsx)(n.code, { children: `true` }), `, sẽ áp dụng ngay thuộc tính `, (0, c.jsx)(n.code, { children: `background-image` }), `.`] }), `
`] }), `
`, (0, c.jsx)(n.h2, { id: `ví-dụ`, children: `Ví dụ` }), `
`, (0, c.jsx)(n.pre, { children: (0, c.jsx)(n.code, { className: `language-scss`, children: `@use '@dxtmisha/styles';

.bg-gradient-faded {
  @include styles.gradientOpacity(0.5);
}

.bg-gradient-faded-important {
  @include styles.gradientOpacity(var(--my-opacity), true, true);
}
` }) })] })
} function s(e = {}) { let { wrapper: n } = { ...t(), ...e.components }; return n ? (0, c.jsx)(n, { ...e, children: (0, c.jsx)(o, { ...e }) }) : o(e) } var c; e(() => { c = n(), a(), r() })(); export { s as default }
