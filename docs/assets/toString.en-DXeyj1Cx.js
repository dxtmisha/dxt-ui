import { i as e } from './preload-helper-Cv19cBWa.js'; import { a as t, t as n } from './jsx-runtime-D1H6-shM.js'; import { c as r, f as i, s as a } from './blocks-CKlddtvc.js'; import { t as o } from './mdx-react-shim-B1U8VZv0.js'; function s(e) {
  let n = { code: `code`, h1: `h1`, h2: `h2`, li: `li`, p: `p`, strong: `strong`, ul: `ul`, ...t(), ...e.components }; return (0, l.jsxs)(l.Fragment, { children: [(0, l.jsx)(a, { title: `@dxtmisha/en/functional-basic/Functions/toString - Conversion of a value to a string` }), `
`, (0, l.jsx)(n.h1, { id: `tostring`, children: (0, l.jsx)(n.code, { children: `toString` }) }), `
`, (0, l.jsx)(n.p, { children: `A utility for converting a value to a string. Returns an empty string if the value is null or undefined.` }), `
`, (0, l.jsx)(n.p, { children: (0, l.jsx)(n.strong, { children: `Parameters:` }) }), `
`, (0, l.jsxs)(n.ul, { children: [`
`, (0, l.jsxs)(n.li, { children: [(0, l.jsx)(n.code, { children: `value: string | null | undefined` }), ` — The value for conversion.`] }), `
`] }), `
`, (0, l.jsxs)(n.p, { children: [(0, l.jsx)(n.strong, { children: `Returns:` }), `
`, (0, l.jsx)(n.code, { children: `string` }), ` — The string representation of the value.`] }), `
`, (0, l.jsx)(n.h2, { id: `examples`, children: `Examples` }), `
`, (0, l.jsx)(r, { language: `typescript`, code: `
import { toString } from '@dxtmisha/functional-basic'

toString('Hello')         // 'Hello'
toString(null)            // ''
toString(undefined)       // ''

` })] })
} function c(e = {}) { let { wrapper: n } = { ...t(), ...e.components }; return n ? (0, l.jsx)(n, { ...e, children: (0, l.jsx)(s, { ...e }) }) : s(e) } var l; e(() => { l = n(), o(), i() })(); export { c as default }
